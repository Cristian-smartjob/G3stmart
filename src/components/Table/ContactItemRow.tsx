...
export default function ContactItemRow({ item, onPressItem }: Props) {
  const isEditingLoading = useSelector<RootState, { [key: string]: boolean }>(
    (state) => state.contacts.isEditingLoading
  );
  const isEditing = isEditingLoading[item.id];

  const isDeletingLoading = useSelector<RootState, { [key: string]: boolean }>(
    (state) => state.contacts.isDeletingLoading
  );
