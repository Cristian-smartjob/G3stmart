...
export default function ComboboxComponent({ list, title, onSelect }: Props) {

  const [query, setQuery] = useState('')
  const [selectedPerson, setSelectedPerson] = useState<ComboBoxItem | null>(null)
    ...
    >
      ...
      <div className="relative mt-2">
        <ComboboxInput
          ...
          onBlur={() => setQuery('')}
          displayValue={(person: ComboBoxItem) => person?.label}
        />
        ...
        {filteredPeople.length > 0 && (
          <ComboboxOptions className="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm">
            {filteredPeople.map((person) => (
              ...
              >
                <span className="block truncate group-data-[selected]:font-semibold">{person.label}</span>
