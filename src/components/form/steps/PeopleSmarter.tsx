...
export default function PeopleSmarter({handleBlur, handleChange}: Props){
    ...
    const jobTitles = genericDataMap[DataTables.JobTitle]
    const seniorities = genericDataMap[DataTables.Seniority]
    ...
    return (
        ...
        <section aria-labelledby="contact-info-heading">
            ...
            <Selector 
                    title='Cargo' 
                    items={(jobTitles || []).map(item => ({id: item.id, label: item.name}))}
                    onChange={() => {
           ...
           <Selector 
                   title='Seniority' 
                   items={(seniorities || []).map(item => ({id: item.id, label: item.name}))}
                   onChange={() => {
