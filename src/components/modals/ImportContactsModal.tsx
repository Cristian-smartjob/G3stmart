...
const ImportContactsModal = ({ isOpen, onClose }: ImportContactsModalProps) => {
  ...
  const readExcelPreview = async (file: File) => {
    ...
    try {
      ...
      const response = await window.fetch("/api/contacts/import", {
        method: "POST",
        body: formData,
      });
  ...
  const handleConfirmImport = async () => {
    ...
    try {
      ...
      if (result.success) {
        // Recargar datos de contactos después de una importación exitosa
        dispatch(fetch());
      }
    } catch (error) {
  ...
  const renderSelectStep = () => (
    <div className="flex flex-col items-center justify-center p-6">
      <h2 className="text-xl font-semibold mb-4">Cargar Excel de Contactos</h2>
      <p className="text-gray-500 text-sm mb-6">
        Selecciona un archivo Excel (.xlsx, .xls) que contenga los datos de los contactos.
      </p>
    ...
    return (
      <div className="p-6">
        ...
        {importResult.success ? (
          <div className="bg-green-50 p-4 rounded-md border border-green-200 mb-6">
            <div className="flex">
              ...
              <div className="ml-3">
                <p className="text-sm font-medium text-green-800">La importación ha sido exitosa!</p>
          ...
          <div className="bg-red-50 p-4 rounded-md border border-red-200 mb-6">
            <div className="flex">
              ...
              <div className="ml-3">
                <p className="text-sm font-medium text-red-800">Ha ocurrido un error durante la importación</p>
        ...
        {importResult.failed && importResult.failed.length > 0 && (
          <div className="mt-4">
            <h3 className="text-lg font-medium text-gray-900">Errores de importación</h3>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {importResult.failed.map((fail, index) => (
                      <tr key={index} className={index % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                        <td className="px-3 py-2 whitespace-nowrap text-xs text-gray-500">{fail.row}</td>
                        <td className="px-3 py-2 text-xs text-gray-500 break-all">{fail.error}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        {importResult.omittedRows && importResult.omittedRows.length > 0 && (
          <div className="mt-4">
            <h3 className="text-lg font-medium text-gray-900">Filas omitidas</h3>
            <div className="mt-2 border rounded-md overflow-hidden">
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {importResult.omittedRows.map((omitted, index) => (
                      <tr key={index} className={index % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                        <td className="px-3 py-2 whitespace-nowrap text-xs text-gray-500">{omitted.row}</td>
                        <td className="px-3 py-2 text-xs text-gray-500 break-all">{omitted.reason}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
